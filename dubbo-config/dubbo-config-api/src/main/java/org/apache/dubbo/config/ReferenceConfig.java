...
public class ReferenceConfig<T> extends ReferenceConfigBase<T> {
    ...
    /**
     * 核心逻辑是调用createProxy()方法
     */
    public synchronized void init() {
        if (initialized) { //检测ReferenceConfig的初始状态
            return;
        }
...
//                    consumerModel.getMethodModel(methodConfig.getName()).addAttribute(ASYNC_KEY, asyncMethodInfo);
        ...
        map.put(REGISTER_IP_KEY, hostToRegistry); //添加ip参数
    ...
    /**
     *createProxy() 方法中处理了多种服务引用的场景，例如，直连单个/多个Provider、单个/多个注册中心。
     * 下面是 createProxy() 方法的核心流程，大致可以梳理出这么 5 个步骤。
     * 	1. 根据传入的参数集合判断协议是否为 injvm 协议，如果是，直接通过 InjvmProtocol 引用服务。
     * 	2. 构造 urls 集合。Dubbo 支持直连 Provider和依赖注册中心两种服务引用方式。如果是直连服务的模式，
     * 	    我们可以通过 url 参数指定一个或者多个 Provider 地址，会被解析并填充到 urls 集合；
     ...
     * 	4. 如果 urls 集合中有多个注册中心，则使用 ZoneAwareCluster 作为 Cluster 的默认实现，生成对应的 Invoker 对象；
     ...
     *
     * 	createProxy() 方法中有两个核心：一是通过 Protocol 适配器选择合适的 Protocol 扩展实现创建 Invoker 对象；
    ...
    private T createProxy(Map<String, String> map) {
        if (shouldJvmRefer(map)) { //根据url的协议、scope以及injvm等参数检测是否需要本地引用
            ...
            if (url != null && url.length() > 0) { // user specified URL, could be peer-to-peer address, or register center's address.
                String[] us = SEMICOLON_SPLIT_PATTERN.split(url); //配置多个URL的时候，用分号进行切分
            ...
            if (urls.size() == 1) {
                //在单注册中心或直连单个服务提供方的时候，通过Protocol的适配器选择对应的Protocol实现创建Invoker对象
                invoker = REF_PROTOCOL.refer(interfaceClass, urls.get(0));
            } else {
                //多注册中心或直连多个服务提供方的时候，会根据每个URL创建Invoker对象
                ...
                if (registryURL != null) { // registry url is available
                    ...
                    //多注册中心场景中，会使用ZoneAwareCluster作为Cluster默认实现，多注册中心之间的选择
