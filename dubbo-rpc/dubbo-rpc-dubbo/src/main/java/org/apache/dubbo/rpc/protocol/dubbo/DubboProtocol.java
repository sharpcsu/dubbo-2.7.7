...
public class DubboProtocol extends AbstractProtocol {
    ...
    /**
     * 首先，在 openServer() 方法中会根据 URL 判断当前是否为服务端，只有服务端才能创建 ProtocolServer 并对外服务。
     ...
     *      在 Server 关闭的时候，只能发送 ReadOnly 请求，这些 ReadOnly 请求由这里设置的
     *      CHANNEL_READONLYEVENT_SENT_KEY 参数值决定是否需要等待响应返回。
     * 3. CODEC_KEY 参数值，默认值为 dubbo。你可以回顾 Codec2 接口中 @Adaptive 注解的参数，都是获取该 URL 中的 CODEC_KEY 参数值。
     * 4. 检测 SERVER_KEY 参数指定的扩展实现名称是否合法，默认值为 netty。你可以回顾 Transporter 接口中 @Adaptive 注解的参数，
     *      它决定了 Transport 层使用的网络库实现，默认使用 Netty 4 实现。
     * 5. 检测 CLIENT_KEY 参数指定的扩展实现名称是否合法。同 SERVER_KEY 参数的检查流程。
     */
    ...
    public <T> Invoker<T> protocolBindingRefer(Class<T> serviceType, URL url) throws RpcException {
        ...
        //将创建的DubboInvoker对象添加到invoker集合中
    ...
    /**
     * 创建共享连接
     * 首先从 referenceClientMap 缓存（Map> 类型）中查询 Key（host 和 port 拼接成的字符串）对应的共享 Client 集合，
    ...
    private List<ReferenceCountExchangeClient> getSharedClient(URL url, int connectNum) {
        ...
        synchronized (locks.get(key)) {  //针对指定地址的客户端进行加锁，分区加锁可以提高并发度
            ...
            // dubbo check
            //再次检查客户端是否全部可用，dubbo中的synchronizd都是这种用法
            if (checkClientCanUse(clients)) {
                //增加应用Client的次数
            ...
            //如果当前Clients集合为空，则直接通过initClient()方法初始化所有的共享客户端
            ...
            /*
             * 清理locks集合中的锁对象，防止内存泄漏，
             * 如果key对应的服务宕机或是下线，不进行清理，用于加锁的Object对象时无法被GC的，从而出现内存泄漏
