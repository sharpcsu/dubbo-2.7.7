...
final public class MockInvoker<T> implements Invoker<T> {
     ...
     * mock 参数以 throw 开头：直接抛出异常。如果在 mock 参数中没有指定异常类型，
     *      则抛出 RpcException，否则抛出指定的 Exception 类型。
     * mock 参数为 true 或 default 时，会查找服务接口对应的 Mock 实现；如果是其他值，
     *      则直接作为服务接口的 Mock 实现。拿到 Mock 实现之后，转换成 Invoker 进行调用。
     */
    @Override
    public Result invoke(Invocation invocation) throws RpcException {
        ...
        //获取mock值（从URL中的methodName.mock参数或mock参数获取）
        String mock = null;
    ...
    /**
     * 检查 mockService 参数是否为 true 或 default，如果是的话，则在服务接口后添加 Mock 字符串，
     * 作为服务接口的 Mock 实现；如果不是的话，则直接将 mockService 实现作为服务接口的 Mock 实现。
     */
    @SuppressWarnings("unchecked")
