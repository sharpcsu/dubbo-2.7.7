...
public class MockClusterInvoker<T> implements Invoker<T> {
    ...
    /**
     * 先判断是否需要开启 Mock 机制。
     * 如果在 mock 参数中配置的是 force 模式，则会直接调用 doMockInvoke() 方法进行 mock。
    ...
    public Result invoke(Invocation invocation) throws RpcException {
        ...
        if (value.length() == 0 || "false".equalsIgnoreCase(value)) {
            //no mock
            //若mock参数未配置或是配置为false，不会开启mock机制，直接调用底层的Invoker
            ...
            //fail-mock
            //如果mock配置不是force，则是fail，会继续调用Invoker对象的invoke()方法进行请求
            try {
                result = this.invoker.invoke(invocation);
                ...
                if(result.getException() != null && result.getException() instanceof RpcException){
                    ...
                    if(rpcException.isBiz()){  //如果是业务异常，会直接抛出
                        ...
                        //如果是非业务异常，调用doMockInvoke()方法返回mock结果
    ...
    /**
     * 调用 selectMockInvoker() 方法获取 MockInvoker 对象，并调用其 invoke() 方法进行 mock 操作
     */
